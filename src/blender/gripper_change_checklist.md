...
그리고 `hub_pick_place.py`의 `GRASP_LATERAL_M` (→ pre-grasp / pocket hover / hub 오프셋 모두 따라감).
...
## 6. 박스 phantom (박스 크기/장착 위치 바뀔 때만) — `hub_pick_place.py`
...
- 수평 그랩이면 "하강"이 공구축 따라 수평 전진으로:
  ...
  `linear_servo([0,0,-descend...])` 축 변경(hub_pick_place의 descend/place-descend 포함).

## R5. 허브 + "두 방향 사이클" — `hub_pick_place.py`
