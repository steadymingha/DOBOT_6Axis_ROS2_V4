...
class ConstrainedPlanner:
    ...
    def lift_path(self, start_q, dz, is_valid_fn, bounds,
                  step=0.003, margin=math.radians(2.0), max_substeps=120,
                  damp=1e-3):
        """Straight vertical lift by +dz along world/base z (sign = direction),
        holding the current orientation. Thin wrapper over linear_path; kept for
        callers that only move vertically. Returns (path, reached_dz, reason)."""
        return self.linear_path(start_q, [0.0, 0.0, dz], is_valid_fn, bounds,
                                step=step, margin=margin, max_substeps=max_substeps,
                                damp=damp)
