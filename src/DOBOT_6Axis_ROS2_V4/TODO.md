...
geometry/config/planner/diagnostics/cycle into separate modules, delete the dead
`move_constrained` method, and move the diagnostic-only `log_gripper_box_clearance`
into its own module. Behaviour stays identical; `cbirrt_pick_place.py` becomes a
thin entrypoint.
...
src/DOBOT_6Axis_ROS2_V4/
  ...
  pick_place/
    ...
    planner.py                # CBiRRTPickPlace class (move_constrained removed)
...
- [ ] DELETE the dead `move_constrained` method (not called anywhere in this file;
      only the old `cbirrt_pick_place_draft.py` used it).
...
(phantom collision, jaw-align, attach/lumping, contact tuning) and two open items:
grasp jitter at close, and the carry (step 7) using plain RRT instead of an
orientation-constrained planner — the latter conflicts with the "delete
move_constrained" task above, resolve before refactoring.
...
## 참고사항
- Decision: subpackage folder layout (not flat modules); delete dead `move_constrained`;
  separate the diagnostic function. (User-confirmed.)
- `move_constrained` uses CBiRRT planning but the live cycle replaced it with
  RRT + linear servo + J6 twist, so it is safe to drop here.
