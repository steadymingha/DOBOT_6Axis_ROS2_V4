...
# Gripper / tool geometry single source of truth: cr7_pnp/gripper_params.py. It is
# a PURE module (math only, no ROS), loaded as a top-level module straight from
# its directory so importing it does NOT trigger cr7_pnp/__init__ (which pulls in
# rclpy) -- this script must run under the bare venv with no ROS sourced. The same
# file feeds the motion package (cr7_pnp.geometry / model), so the gripper, the
# box and the tool offsets are defined ONCE for both.
sys.path.insert(0, os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'cr7_pnp'))
from gripper_params import (  # noqa: E402
    ARM_JOINTS, GRIPPER_JOINTS, EE_FRAME, GRASP_CENTER_OFFSET_M)
...
# Combined AGV+base+gripper model used by run_mpo700_cr7.sh. With --combined the
# map locks every non-arm joint (AGV body, cube platform, gripper) into fixed
# attached geometry, so the arm's reach is checked against the real deployed
# robot. Its model root is mpo_base_link (~floor), so reported TCP z is height
# above the floor -- i.e. the shelf height a down-pointing gripper can reach.
DEFAULT_COMBINED_XACRO = os.path.expanduser(
    '~/dobot_ws/install/cra_description/share/cra_description/urdf/cr7_on_mpo700.urdf.xacro')
# tools/ is one level below the package root; SRDF + outputs live at the root.
_PKG_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DEFAULT_SRDF = os.path.join(_PKG_ROOT, 'cr7_moveit', 'config', 'cr7_robot.srdf')

# ARM_JOINTS, GRIPPER_JOINTS, EE_FRAME imported from gripper_params (above).
...
# TCP used by this reach map = the GRASP CENTRE (box centre when grasped), i.e.
# Link6 + GRASP_CENTER_OFFSET_M (=0.135) along the tool z-axis, from gripper_params.
# This makes a reported z the graspable magazine-centre HEIGHT -- a physically
# meaningful point -- instead of the abstract IK-target TCP (0.12005), which floats
# ~38 mm below the jaw tips. NOTE: the grasp centre also hangs GRASP_LATERAL_M
# (~140 mm) off the tool axis; pointing straight down that is purely horizontal, so
# it shifts the grasp x,y but NOT the height this map reports.
TCP_OFFSET_M = GRASP_CENTER_OFFSET_M
...
def _worker_init(xacro, srdf, limits_rad, orientations, down_flags, params,
                 base_seed, combined=False):
    global _W_RM, _W_ORI, _W_DOWN, _W_PARAMS, _W_SEED
    _W_RM = ReachabilityModel(xacro, srdf, lock_non_arm=combined,
                              arm_pairs_only=combined)
...
def compute_reachability(rm, centers, orientations, down_flags, args,
    ...
    if jobs == 1:
        ...
        init_args = (args.xacro, args.srdf, limits_rad, orientations,
                     down_flags, params, base_seed, args.combined)
        done = 0
        na = len(active)
...
def parse_args(argv=None):
    ...
    p.add_argument('--xacro', default=None, help='robot xacro path (default: '
                   'bare arm, or the combined AGV model with --combined)')
    p.add_argument('--combined', action='store_true',
                   help='use the combined AGV+base+gripper model '
                        '(run_mpo700_cr7.sh): lock all non-arm joints as fixed '
                        'attached geometry. Root frame is mpo_base_link (~floor), '
                        'so reported TCP z is height above the floor.')
    ...
    p.add_argument('--out-dir', default=os.path.join(_PKG_ROOT, 'reachability_out'),
...
def main(argv=None):
    ...
    if args.xacro is None:
        args.xacro = DEFAULT_COMBINED_XACRO if args.combined else DEFAULT_XACRO

    limits_deg, limits_rad = resolve_limits(args)
    bounds = resolve_bounds(args)

    kind = 'combined AGV+base+gripper' if args.combined else 'bare arm'
    print(f'[reach] loading model ({kind}): {args.xacro}', flush=True)
    rm = ReachabilityModel(args.xacro, args.srdf, lock_non_arm=args.combined,
                           arm_pairs_only=args.combined)
