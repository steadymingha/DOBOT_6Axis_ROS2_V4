...
### 6-8. ★ 부채: `.venv` 의 cv2 더미 (2026-08-10)

`test/vision_target.py` 는 **pinocchio 가 있는 `.venv`** 로 돌아야 하는데, 그 venv 는
numpy 2.2.6 이고 컨테이너의 cv2 4.5.4 는 numpy 1.x 로 빌드돼 있어 `import cv2` 자체가
깨진다. 이 경로가 쓰는 것은 `handeye_calib` 의 `tool_to_T` / `STILL_DPS` 뿐이고 둘 다 순수
numpy 인데, 그 모듈이 **import 시점에** cv2 상수(`METHODS`)를 참조한다. 남의 모듈을 고치지
않고 지나가려고 `vision_target.py` 가 실패 시 `sys.modules['cv2']` 에 `MagicMock` 을 끼운다.

> **이 venv 에서 cv2 를 실제로 쓰는 코드를 돌리면 조용히 오동작한다.**
> 지금은 `tool_to_T` 만 타므로 무해하지만, 이 프로세스에 cv2 를 호출하는 코드가 들어오면
> 반환값이 MagicMock 이 되어 계산 결과가 숫자가 아니게 된다.

**항구 해결**: `handeye_calib.py` 에서 **순수 수학부(`robot_math`)를 분리**한다 —
`tool_to_T` / `Rx,Ry,Rz` / `rot_angle` / `make_T` / `RobotFeed` / 오프셋 상수는 cv2 와 무관
하므로 별 모듈로 빼고, cv2 를 쓰는 캘리브 본체만 남긴다. 그러면 `transform.py` 도
`vision_target.py` 도 cv2 를 통째로 건너뛴다.
**다음에 `handeye_calib.py` 를 수정할 일이 생길 때 같이 할 것** (그 전에는 건드리지 않는다 —
검증된 파일이고, 지금 더미로 실제 손해가 없다).
...
| **cv2 더미 제거** | 6-8. `handeye_calib` 에서 순수 수학부(`robot_math`)를 분리. 그 파일을 다음에 고칠 때 같이 한다 |
