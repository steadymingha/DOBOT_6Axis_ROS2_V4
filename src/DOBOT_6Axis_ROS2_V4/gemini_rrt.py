...
class CR7RRTPlanner(Node):
    ...
    # def get_ik(self, target_pose: PoseStamped):
    #     """Convert the target pose into joint angles (IK)"""
    ...
    #     # Wait in a thread-safe way instead of spin_until_future_complete
    ...
    # --- [RRT algorithm core logic] ---

    # def plan_rrt(self, start_joints, goal_joints, max_iter=5000, step_size=1):#0.5):
    ...
    #     # # joint_limits = [(-math.pi, math.pi) for _ in range(len(start_joints))]
    #     # joint_limits = [
    #     #     (math.radians(-101), math.radians(10)),  # Joint 1: -101deg ~ 10deg (restrict X+ region)
    #     #     (math.radians(-70), math.radians(60)),                     # Joint 2: no limit (adjust later)
    #     #     (-math.pi, math.pi),                     # Joint 3: no limit
    #     #     (-math.pi, math.pi),                     # Joint 4: no limit
    #     #     (-math.pi, math.pi),                     # Joint 5: no limit
    #     #     (-math.pi, math.pi)                      # Joint 6: no limit
    #     # ]
    ...
    #         else:
    #             q_rand = np.array([random.uniform(limit[0], limit[1]) for limit in joint_limits])
    ...
    #         if self.is_state_valid(q_new_joints.tolist()):
    ...
    def execute_trajectory(self, path):
        """Send the planned path to the action server"""
